...
fn std(args: StdArgs) -> String {
	...
	if args.to_convert < 20.0 {
		...
		format!("{}%\npart above the section's upper boundary: 1/{}", cp * 100.0, (2.0 / (1.0 - cp)).round())
	// 0.5 because here we normally want to know what it takes to exit the curve from the upper bound.
